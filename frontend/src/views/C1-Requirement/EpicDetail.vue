  ...
  <div class="epic-detail-container" v-loading="loading">
    ...
    <el-tabs v-model="activeTab" class="epic-tabs">
      ...
      <el-tab-pane name="review">
        <template #label>
          Epic评审 
          <el-badge v-if="epic?.reviewStatus" :value="getReviewStatusText(epic.reviewStatus)" :type="getReviewStatusType(epic.reviewStatus)" />
        </template>
        
        <el-card shadow="never">
          <!-- 评审状态 -->
          <el-alert
            v-if="epic?.reviewStatus"
            :title="`评审状态: ${getReviewStatusText(epic.reviewStatus)}`"
            :type="getReviewStatusType(epic.reviewStatus)"
            :closable="false"
            style="margin-bottom: 20px;"
          >
            <template v-if="epic.reviewStatus === 'approved'">
              Epic评审已通过，可以继续执行
            </template>
            <template v-else-if="epic.reviewStatus === 'rejected'">
              Epic评审被拒绝，请根据评审意见修改后重新提交
            </template>
            <template v-else-if="epic.reviewStatus === 'in-review'">
              Epic正在评审中，请等待评审人员反馈
            </template>
            <template v-else>
              Epic尚未提交评审，请完善内容后提交
          ...
          <div class="review-actions" style="margin-bottom: 20px;">
            <el-button
              v-if="!epic?.reviewStatus || epic.reviewStatus === 'pending' || epic.reviewStatus === 'rejected'"
              type="primary"
              :icon="Check"
            ...
            <el-button
              v-if="epic?.reviewStatus === 'in-review'"
              type="success"
              :icon="Check"
          ...
          <el-timeline v-if="epic?.reviewComments && epic.reviewComments.length > 0">
            <el-timeline-item
              v-for="comment in epic.reviewComments"
              :key="comment.id"
...
const handleApproveReview = async () => {
  try {
    await ElMessageBox.prompt('请输入批准意见', '批准Epic', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
...
const handleRejectReview = async () => {
  try {
    await ElMessageBox.prompt('请输入拒绝原因', '拒绝Epic', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
