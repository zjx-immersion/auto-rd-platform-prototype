...
│  │ │ 未分配SSTS (12个)                                            │   │  │
│  │ │ ┌─────────────────────────────────────────────────────────┐ │   │  │
│  │ │ │ [SSTS-001] ACC目标车辆检测              [拖动] [管理依赖] │ │   │  │
│  │ │ │ 故事点: 13 SP │ 优先级: 高 │ Feature: FEAT-001          │ │   │  │
│  │ │ │ 团队: 未分配 │ 依赖: 2个 (SSTS-003, SSTS-005)          │ │   │  │
│  │ │ │ 💡 点击卡片查看依赖关系                                  │ │   │  │
│  │ │ └─────────────────────────────────────────────────────────┘ │   │  │
│  │ └───────────────────────────────────────────────────────────────┘   │  │
│  └─────────────────────────────────────────────────────────────────────┘  │
...
│  │ Sprint里程碑设置                                                    │  │
│  │ ┌───────────────────────────────────────────────────────────────┐  │  │
│  │ │ Sprint-1 (01/01-14) │ [设置里程碑] [编辑]                    │  │  │
│  │ │   🎯 产品版本: v1.2.0 │ 🚗 整车节点: M1 (功能验证)          │  │  │
│  │ │   里程碑日期: 2026-01-14 │ 类型: 产品发布                    │  │  │
│  │ │   💡 指导: 该Sprint需完成v1.2.0版本的关键Feature             │  │  │
│  │ │                                                               │  │  │
│  │ │ Sprint-2 (01/15-28) │ [设置里程碑] [编辑]                    │  │  │
│  │ │   🎯 产品版本: v1.3.0 │ 🚗 整车节点: M2 (集成测试)          │  │  │
│  │ │   里程碑日期: 2026-01-28 │ 类型: 集成测试                    │  │  │
│  │ │   💡 指导: 该Sprint需完成集成测试准备                         │  │  │
│  │ │                                                               │  │  │
...
- 依赖的SSTS卡片显示高亮边框（蓝色表示已分配，黄色表示未分配）
- 如果依赖的SSTS已分配，显示连线或箭头指向其位置（团队×Sprint）
- 如果依赖的SSTS未分配，显示警告提示和"未分配"标签
- 依赖关系面板显示详细信息：
  - 依赖的SSTS列表（该SSTS依赖哪些SSTS）
  - 被依赖的SSTS列表（哪些SSTS依赖该SSTS）
  - 依赖类型（强依赖/弱依赖/可选依赖）
  - 依赖原因和详情
  - 依赖SSTS的分配状态（已分配/未分配，位置信息）
- 点击SSTS卡片时，自动显示依赖关系面板
- 支持在依赖关系面板中直接跳转到依赖的SSTS位置

#### 4. SSTS依赖关系管理

**添加依赖关系**:
1. 选中某个SSTS卡片
2. 点击"添加依赖关系"按钮（在依赖关系面板中）
3. 在弹出的对话框中：
   - **选择要依赖的SSTS**（从列表中选择，支持搜索）
   - **选择依赖类型**：
     - 强依赖：必须完成依赖的SSTS才能开始
     - 弱依赖：建议先完成依赖的SSTS，但不是必须
     - 可选依赖：可选，不影响执行顺序
   - **输入依赖原因**（为什么需要这个依赖，必填）
   - **输入依赖详情**（具体的依赖内容，可选）
   - 预览依赖关系图
...
**设置里程碑**:
1. 点击Sprint的"设置里程碑"按钮（在Sprint列标题或里程碑设置面板中）
2. 在弹出的对话框中：
   ...
   - **选择里程碑类型**：
     ...
     - 测试：测试里程碑
     - 其他：自定义类型
   - **输入里程碑描述**（可选，如：关键功能验证点）
   - **设置里程碑日期**（默认使用Sprint结束日期，可调整）
3. 保存里程碑
4. Sprint列标题显示里程碑图标和简要信息
...
- **根据里程碑要求，推荐Feature/SSTS的分配**
  - 如果Sprint有产品版本里程碑，推荐该版本的关键Feature
  - 如果Sprint有整车节点里程碑，推荐该节点相关的Feature/SSTS
- **里程碑日期前必须完成关键Feature/SSTS**
  - 显示里程碑相关的Feature/SSTS列表
  - 标记必须在里程碑前完成的Feature/SSTS
- **里程碑倒计时提醒**
  - 显示距离里程碑日期的剩余时间
  - 如果时间紧张，显示警告提示
- **里程碑约束检查**
  - 分配Feature/SSTS时，检查是否符合里程碑要求
  - 如果不符合，提示用户调整分配
