...
PI Planning 2阶段规划工作台是PI Planning的核心功能，支持两个阶段的规划流程：

**阶段1（Feature/SSTS排布）**: 将Feature和SSTS排布到不同的Sprint（迭代）和团队下，形成整体的PI计划。

**阶段2（模块需求排布）**: 团队基于阶段1的结果，站在单个团队迭代的视角，拆解/排布已有的模块需求（MR）到不同的迭代。
...
```
┌─────────────────────────────────────────────────────────────────────────────┐
│  PI Planning 2阶段规划工作台 - PI-2026-Q1                                   │
├─────────────────────────────────────────────────────────────────────────────┤
│  [←返回] [阶段1: Feature/SSTS排布] [阶段2: 模块需求排布] [保存草稿] [提交] │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  PI信息卡片                                                                 │
│  ┌─────────────────────────────────────────────────────────────────────┐  │
...
2. 拖动到左侧某个Sprint中
3. 释放鼠标，系统执行以下检查：
   - ✅ 容量检查：该Sprint的可用容量是否足够（考虑Feature/SSTS已占用的容量）
   - ✅ 依赖检查：依赖的MR是否已分配
   - ✅ 技能检查：团队是否具备所需技能
...
- [ ] 能够切换不同团队进行规划
- [ ] 能够显示该团队的所有Sprint（来自阶段1）
- [ ] 能够显示该团队待分配的MR列表
- [ ] 支持拖拽MR到Sprint
- [ ] 容量检查：考虑Feature/SSTS已占用容量
- [ ] 依赖检查：依赖未分配时禁止分配并提示
- [ ] 实时更新Sprint负载（Feature/SSTS + MR）
- [ ] 支持移除已分配的MR
- [ ] 支持批量分配MR
- [ ] 支持保存草稿和提交阶段2
...
1. **两阶段分离**: 阶段1关注整体规划，阶段2关注团队详细规划
2. **可视化优先**: 使用看板视图，支持拖拽操作
3. **实时反馈**: 容量、负载率实时更新，及时预警
4. **依赖管理**: 自动检查依赖关系，确保规划合理性
5. **团队自主**: 阶段2让团队自主规划，提高参与度
