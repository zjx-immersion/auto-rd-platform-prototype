...
### 第8层：Stage1/Stage2 Sprint和团队显示问题 ✅

**症状**: 两个新问题

**页面**: 
- PI Planning - 全局视角（PIPlanningStage1.vue）
- PI Planning - 团队视角（PIPlanningStage2.vue）

**问题1 - Stage1 Sprint数据未显示**:
- Sprint管理区域为空（应该显示8个Sprint）
- 里程碑配置为空
- 根本原因：Sprint过滤失败（ID大小写不匹配，第4次）

**问题2 - Stage2 页面组件和数据都为空**:
- 团队按钮只显示蓝色色块，没有团队名称
- 页面显示"请先选择一个团队"
- 内容区域完全空白
- 根本原因A：Sprint过滤失败（ID大小写不匹配，第5次）
- 根本原因B：Element Plus语法错误（`:value` → `:label`）

**修复**:
1. Stage1 Sprint过滤使用`toLowerCase()`
2. Stage2 Sprint过滤使用`toLowerCase()`
3. Stage2 团队按钮修正为`:label`属性
4. 添加诊断日志
